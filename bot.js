/**
 * TradeBotIQ v4.6x — Fixed Ed25519 + recvWindow + Fallback Auth
 * ============================================================================
 * Fixes applied:
 * - recvWindow=10000 added to ALL signed requests (binanceRequest + ccxt)
 * - verifyAuth now tries testnet if live fails with -2015
 * - More aggressive time sync and better error messages
 */
...
const privateKeyPem = fsReadFileSync("./private.pem", "utf8").trim();
if (!privateKeyPem.includes("BEGIN PRIVATE KEY")) {
  throw new Error("private.pem does not contain a valid PEM key");
}
const privateKey = createPrivateKey({ key: privateKeyPem, format: "pem" });
console.log(`🔑 Key loaded from file — type: ${privateKey.asymmetricKeyType}`);
...
async function syncServerTime() {
  try {
    const res  = await fetch(`${CONFIG.apiBase}/api/v3/time`);
    const data = await res.json();
    serverTimeOffset = data.serverTime - Date.now();
    const absMs = Math.abs(serverTimeOffset);
    console.log(`   🕐 Server time synced | Offset: ${serverTimeOffset}ms ${absMs > 1000 ? "⚠️ HIGH DRIFT" : "✅"}`);
  } catch (err) {
    console.log(`   ⚠️ Time sync failed: ${err.message} — using local time`);
    serverTimeOffset = 0;
  }
}
...
function assetDelay() {
  return CONN.fastMode
    ...
    : Math.floor(Math.random() * 501) + 1200;
}

function requestDelay() {
  return CONN.fastMode
    ? Math.floor(Math.random() * 101) + 200
    : Math.floor(Math.random() * 201) + 400;
}
...
function signRequest(params) {
  ...
  const sig       = cryptoSign(null, Buffer.from(qs), privateKey).toString("base64url");
  return { qs, signature: sig };
}

// ─── Core HTTP (now with recvWindow) ────────────────────────────────────────
async function binanceRequest(method, path, params = {}, signed = true, retries = 5) {
  let url;
  if (signed) {
    // ✅ FIX: Add recvWindow to all signed requests
    const allParams = { ...params, recvWindow: 10000 };
    const { qs, signature } = signRequest(allParams);
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      if (data.code && data.code < 0) {
        if (data.code === -1021 || data.code === -1022) {
          console.log(`   ⏱️ Timestamp error (${data.code}) — re-syncing server time...`);
...
function createCcxtExchange() {
  ...
  ex.sign = function(path, api, method, params, headers, body) {
    ...
    const baseUrl    = IS_LIVE ? "https://api.binance.com" : "https://testnet.binance.vision";
    return {
      url:    `${baseUrl}${path}?${fullQs}`,
      method,
      body:   "",
      headers: {
        "X-MBX-APIKEY": apiKey,
...
// ─── Auth Verification (with testnet fallback for -2015) ────────────────────
async function verifyAuth() {
  CONN.authAttempts++;
  await syncServerTime();

  // Try live first, then testnet if live gives -2015
  const envs = [
    { name: 'live', base: 'https://api.binance.com' },
    { name: 'testnet', base: 'https://testnet.binance.vision' }
  ];

  for (const env of envs) {
    const originalBase = CONFIG.apiBase;
    CONFIG.apiBase = env.base;

    try {
      console.log(`   🔐 Auth attempt ${CONN.authAttempts} on ${env.name} (offset: ${serverTimeOffset}ms)...`);
      await binanceRequest("GET", "/api/v3/account", {}, true, 1);
      CONN.authenticated = true;
      CONN.fastMode      = true;
      CONN.lastAuthTime  = new Date().toISOString();
      CONN.ccxtExchange  = createCcxtExchange();
      console.log(`   ✅ Auth confirmed on ${env.name} — FAST MODE activated`);
      console.log(`   ⚡ ccxt exchange ready (1.3s REST)`);
      CONFIG.apiBase = env.base; // keep this environment
      return true;
    } catch (err) {
      CONFIG.apiBase = originalBase;
      console.log(`   ❌ Auth failed on ${env.name}: ${err.message}`);
      if (err.message.includes('-2015') && env.name === 'live') {
        console.log(`   ℹ️  -2015 often means IP restriction or key not active. Trying testnet...`);
        continue;
      }
      break; // for other errors, don't try testnet
    }
  }
  ...
  console.log(`   ⏳ recWind: ${(w/1000).toFixed(1)}s (range ${CONFIG.recWindMin}–${CONFIG.recWindMax}s)`);
...
async function placeMarketOrder(symbol, side, quantity) {
  ...
  const qtyStr  = qty.toFixed(PRECISION[symbol]?.qty || 4);
  const ccxtSym = symbol.replace("USDT", "/USDT");

  if (CONN.ccxtExchange && !CONFIG.paperTrading) {
    if (side.toUpperCase() === "BUY") {
      return CONN.ccxtExchange.createMarketBuyOrder(ccxtSym, parseFloat(qtyStr));
    } else {
      return CONN.ccxtExchange.createMarketSellOrder(ccxtSym, parseFloat(qtyStr));
    }
  }

  return binanceRequest("POST", "/api/v3/order", {
    symbol, side: side.toUpperCase(), type: "MARKET", quantity: qtyStr,
  }, true);
}
...
function formatQty(symbol, qty) {
  ...
  return Math.max(parseFloat(qty.toFixed(p.qty)), p.minQty);
}
...
function calcRealPnL(pos, exitPrice) {
  ...
  const slip    = (pos.sizeUSD+exitTot)*CONFIG.slippagePct;
  const gross   = exitTot-pos.sizeUSD;
  return {gross, fees, slippage:slip, net:gross-fees-slip};
}
...
function calcStochRSI(closes) {
  ...
  const p1=rv.length>1?((rv[rv.length-2]-minR)/(maxR-minR||1))*100:rawK;
  const p2=rv.length>2?((rv[rv.length-3]-minR)/(maxR-minR||1))*100:rawK;
  return {k:rawK, d:(rawK+p1+p2)/3, oversold:rawK<20, overbought:rawK>80};
}
function calcMACD(closes, fast=12, slow=26, signal=9) {
  ...
  const macdLine=mv[mv.length-1], signalLine=calcEMA(mv,signal);
...
function calcATR(candles, period=14) {
  ...
  const trs=candles.slice(-period-1).map((c,i,arr)=>
    i===0?c.high-c.low:Math.max(c.high-c.low,Math.abs(c.high-arr[i-1].close),Math.abs(c.low-arr[i-1].close)));
...
function calcFibLevels(closes, lookback=50) {
  ...
  const high=Math.max(...recent), low=Math.min(...recent), range=high-low||1;
  const price=closes[closes.length-1];
...
function calcAIScore(d, ob, m5, direction) {
  ...
  if (d.atrPct>5.0) score-=10; else if(d.atrPct>4.0) score-=5;
...
async function analyzeAsset(symbol) {
  ...
  const ema5=calcEMA(closes,5),ema8=calcEMA(closes,8),ema13=calcEMA(closes,13);
  const ema21=calcEMA(closes,21),ema50=calcEMA(closes,50),ema200=calcEMA(closes,200);
  const rsi3=calcRSI(closes,3),rsi14=calcRSI(closes,14);
  const stochRSI=calcStochRSI(closes),macdData=calcMACD(closes);
  const bb=calcBB(closes),vwap=calcVWAP(candles),cmf=calcCMF(candles);
  const rsiDiv=calcRSIDivergence(closes),tdSeq=calcTDSequential(closes);
  const fib=calcFibLevels(closes),atr=calcATR(candles);
  const atrPct=price>0?(atr/price)*100:0;
  const vols=candles.map(c=>c.volume);
  const avgVol20=vols.slice(-20).reduce((a,b)=>a+b,0)/20;
  const volConfirmed=vols[vols.length-1]>avgVol20*1.1;
  const last=candles[candles.length-1],prev=candles[candles.length-2];
  const body=Math.abs(last.close-last.open),wick=(last.high-last.low)||0.0001;
  return {
    symbol,price,candles,closes,ema5,ema8,ema13,ema21,ema50,ema200,
    rsi3,rsi14,stochRSI,macdData,bb,vwap,cmf,rsiDiv,tdSeq,fib,atr,atrPct,volConfirmed,
...
function strategyS1(d,ob,m5) {
  ...
  const buy=setA||setB||setC||setD||setE_buy||setF_buy;
  const sell=setG||setH||setI||setJ;
  let signal=null;
  if (buy&&!sell) signal="BUY"; if (sell&&!buy) signal="SELL";
  return {signal,score:signal?calcAIScore(d,ob,m5,signal):0,strategy:"S1",
    details:{setA,setB,setC,setD,setE_buy,setF_buy,setG,setH,setI,setJ,ribbonBull,ribbonBear,goldenZone,macdBull,macdBear}};
}
...
function strategyS2(d,ob,m5) {
  ...
  const buy=setA2||setB2||setC2||setD2||setE2;
  const sell=setF2||setG2||setH2;
  let signal=null;
  if (buy&&!sell) signal="BUY"; if (sell&&!buy) signal="SELL";
  return {signal,score:signal?calcAIScore(d,ob,m5,signal):0,strategy:"S2",
...
function strategyVolatility(d,ob,m5) {
  ...
  const highest=Math.max(...prev20.map(c=>c.high));
  const lowest=Math.min(...prev20.map(c=>c.low));
...
async function checkExits(symbol, price, botState) {
  ...
  for (const pos of positions) {
    ...
    if (pnlPct>=CONFIG.trailingActivate) {
      const trail=price*(1-CONFIG.trailingStopPct/100);
      if (trail>slPrice) {slPrice=trail; pos.slPrice=slPrice; changed=true;}
    }
    ...
    if (price>=tpPrice||price<=slPrice) {
      ...
      if (!CONFIG.paperTrading) await placeMarketOrder(symbol,"SELL",pos.sizeUSD/price);
      else console.log(`   📋 PAPER EXIT Net:$${pnl.net.toFixed(3)}`);
      ...
      console.log(`   ⏳ [${pos.strategy}] ${symbol} ${pnlPct>=0?"📈":"📉"}${pnlPct.toFixed(2)}% | TP:${fmtPrice(tpPrice)} SL:${fmtPrice(slPrice)}${pnlPct>=CONFIG.trailingActivate?" 🔄":""}`);
      remaining.push(pos);
    }
  }
  if (changed||remaining.length!==positions.length) savePositions([...others,...remaining]);
}
...
async function executeSignal(symbol,signal,strategy,score,price,atr,balance,equityScaler,allocPct,logEntry) {
  ...
  if (existing) {console.log(`   ⚠️  [${strategy}] Open @ ${fmtPrice(existing.entryPrice)} — skip`); return;}
  if (loadPositions().length>=CONFIG.maxPositions) {console.log(`   ⚠️  Max positions — skip`); return;}
  ...
  Object.assign(logEntry,{tradeSize:size,signal,triggerSet:strategy,allPass:true,
  ...
  if (CONFIG.paperTrading) {
    ...
    logEntry.notes=`Paper ${signal} Sc:${score} via ${strategy}`;
    ...
    try {
      ...
      const oid=order.id||order.orderId||String(order);
      Object.assign(logEntry,{orderPlaced:true,orderId:String(oid),side:signal});
      logEntry.notes=`Live ${signal} ID:${oid} Sc:${score} via ${strategy}`;
      console.log(`   ✅ LIVE ${signal} ID:${oid} $${size.toFixed(2)}`);
    } catch(err) {
  ...
  if (logEntry.orderPlaced&&signal==="BUY") {
    ...
    positions.push({symbol,entryPrice:price,sizeUSD:size,strategy,
      timestamp:new Date().toISOString(),tpPrice,slPrice:stopPrice,rr:CONFIG.rrRatio});
...
async function morningBrief() {
  const botState=loadState(), alloc=getOptimizedAllocation(botState);
  const balance=await fetchAccountBalance(), dd=checkDrawdown(balance);
  console.log("\n╔═══════════════════════════════════════════════════════════╗");
  ...
  console.log(`  Auth: ${CONN.authenticated?"✅ Connected":"❌"} | Mode: ${CONN.fastMode?"⚡ FAST":" 🐢"} | TimeOffset: ${serverTimeOffset}ms\n`);
  for (const [s,a] of Object.entries(alloc)) {
    const wr=a.wr!==null?`${(a.wr*100).toFixed(0)}%`:"new";
    console.log(`  ${s.padEnd(16)} WR:${wr.padEnd(5)} Alloc:${a.pct.toFixed(0)}% PnL:$${(botState.totalPnL?.[s]||0).toFixed(2)}`);                                                                          }
  console.log("");
  for (const symbol of CONFIG.assets) {
    ...
    try {
      ...
      const s1=strategyS1(d,ob,m5), s2=strategyS2(d,ob,m5);
      const trend=d.ema5>d.ema8&&d.ema8>d.ema21?"📈 BULL":d.ema5<d.ema8&&d.ema8<d.ema21?"📉 BEAR":"↔️  FLAT";
      console.log(`  ${symbol.padEnd(10)} ${fmtPrice(d.price).padStart(14)} | ${trend} | CMF:${d.cmf.toFixed(3)} | ATR:${d.atrPct.toFixed(2)}%`);
      console.log(`    S1:${s1.signal?`${s1.signal}(Sc:${s1.score})`:"⚪"} | S2:${s2.signal?`${s2.signal}(Sc:${s2.score})`:"⚪"}`);
    } catch(err) {
      console.log(`  ${symbol} ❌ ${err.message}`);
    }
  }
  writeFileSync(BRIEF_FILE,JSON.stringify({timestamp:new Date().toISOString(),balance,mode:CONN.fastMode?"fast":"conservative"},null,2));
  console.log(`\n  Saved → ${BRIEF_FILE}\n`);
}
...
strategy("TradeBotIQ v4.6x",overlay=true,default_qty_type=strategy.percent_of_equity,default_qty_value=1)
...
plot(ema21,color=color.blue,linewidth=2);plot(ema50,color=color.orange,linewidth=2);plot(ema200,color=color.red,linewidth=3)
...
plot(fib618,color=color.new(color.yellow,50),linewidth=1)
plot(fib50,color=color.new(color.yellow,70),linewidth=1)
bullDiv=close<close[5] and ta.rsi(close,14)>ta.rsi(close[5],14)
bearDiv=close>close[5] and ta.rsi(close,14)<ta.rsi(close[5],14)
...
plotshape(s1Buy,location=location.belowbar,color=color.green,style=shape.labelup,text="BUY")
...
alertcondition(s1Buy,"BUY","TradeBotIQ BUY: {{ticker}} @ {{close}}")
...
async function run() {
  ...
  // Auth with recWind retry
  for (let a=0; a<5; a++) {
    if (await verifyAuth()) break;
    if (a===4) {console.log("🛑 Auth failed after 5 attempts — aborting"); return;}
  }
  if (!CONN.authenticated) return;

  const botState=loadState(), alloc=getOptimizedAllocation(botState);
  const balance=await fetchAccountBalance(), equityScaler=getEquityScaler(balance);
  const dd=checkDrawdown(balance);
  const bh=loadBalanceHistory();
  const prevBal=bh.history.length>1?bh.history[bh.history.length-2]?.balance:balance;

  if (!dd.ok) {
    console.log(`🛑 MAX DRAWDOWN (${dd.drawdownPct.toFixed(1)}%) — paused`);
    return;
  }

  const activeList=CONFIG.activeStrategies.includes("all")
    ?["S1","S2","macd","enhanced_macd","bollinger","volatility"]                                         :CONFIG.activeStrategies;

  console.log("═══════════════════════════════════════════════════════════");
  console.log("  TradeBotIQ v4.6x — Extended Edition");
  console.log(`  ${new Date().toISOString()}`);
  console.log(`  Mode:      ${CONFIG.paperTrading?"📋 PAPER":"🔴 LIVE"} | ${IS_LIVE?"🔴 LIVE":"🧪 DEMO"}`);
  console.log(`  Auth:      ✅ Ed25519 | TimeOffset: ${serverTimeOffset}ms`);                          console.log(`  Speed:     ⚡ FAST (800–1200ms per asset)`);
  console.log(`  Orders:    ccxt 1.3s REST`);
  console.log(`  Balance:   $${balance.toFixed(2)} ${(balance-prevBal)>=0?`📈+$${(balance-prevBal).toFixed(2)}`:`📉$${(balance-prevBal).toFixed(2)}`}`);
  console.log(`  DD:        ${dd.drawdownPct.toFixed(1)}% (limit -${CONFIG.maxDrawdown}%) �`);
  console.log(`  Equity:    ${equityScaler.toFixed(2)}x | Pos:${loadPositions().length}/${CONFIG.maxPositions}`);
  ...
  const log=loadLog();
  const stratFns={S1:strategyS1,S2:strategyS2,macd:strategyMACD,enhanced_macd:strategyEnhancedMACD,bollinger:strategyBollinger,volatility:strategyVolatility};
  ...
  for (const symbol of CONFIG.assets) {
    ...
    const trend=d.ema5>d.ema8&&d.ema8>d.ema13&&d.ema13>d.ema21&&d.ema21>d.ema50?"📈 BULL"
               :d.ema5<d.ema8&&d.ema8<d.ema13&&d.ema13<d.ema21&&d.ema21<d.ema50?"📉 BEAR":"↔️  FLAT";
    const zone=d.price>d.ema50&&d.price>d.ema200?"�":"🔴";
    const obStr=ob?`Imb:${ob.imbalance.toFixed(2)}${ob.bullish?"�":ob.bearish?"🔴":""}`:"-";
    const m5Str=m5?(m5.bullish?"📈":m5.bearish?"📉":"↔️"):"-";

    console.log(`\n── ${symbol} ${fmtPrice(d.price)} ─────────────────────────────────────`);
    console.log(`   ${trend} | ${zone} | CMF:${d.cmf.toFixed(3)} | TD:${d.tdSeq.count}${d.tdSeq.upCount>0?"↑":"↓"} | ATR:${d.atrPct.toFixed(2)}%`);
    ...
    console.log(`   OB:${obStr} | 5m:${m5Str} | Fib618:${fmtPrice(d.fib.fib618)} Near:${d.fib.nearFib618?"✅":"🚫"}`);
    ...
    for (const stratName of activeList) {
      ...
      if (res.score<minScore) {console.log(`   ⚠️  ${stratName} ${res.signal} Sc:${res.score}<${minScore} — skip`); continue;}
      if (isDuplicateSignal(symbol,stratName,res.signal)) {console.log(`   ⏭️  ${stratName} duplicate — skip`); continue;}
      if (countTodaysTrades(log,symbol,stratName)>=CONFIG.maxTradesPerDay) {console.log(`   🚫 ${stratName} daily limit`); continue;}
      const allocPct=alloc[stratName]?.pct||CONFIG.s1Pct;
      console.log(`   🎯 ${stratName} ${res.signal} | Sc:${res.score} | Alloc:${allocPct.toFixed(0)}%`);
      const logEntry={timestamp:new Date().toISOString(),symbol,price:d.price,paperTrading:CONFIG.paperTrading,strategy:stratName};
      await executeSignal(symbol,res.signal,stratName,res.score,d.price,d.atr,balance,equityScaler,allocPct,logEntry);                                                                                          log.trades.push(logEntry);
  ...
  saveLog(log);
  saveState(botState);
  console.log(`\n✅ Done | $${balance.toFixed(2)} | Pos:${loadPositions().length}/${CONFIG.maxPositions} | DD:${dd.drawdownPct.toFixed(1)}%`);
  console.log(`   Auth: ✅ Ed25519 | ccxt 1.3s REST | TimeOffset: ${serverTimeOffset}ms`);
  console.log("═══════════════════════════════════════════════════════════\n");
}
